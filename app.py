...
def download_invoice(booking_id):
    ...
    # Footer
    footer_text = "Thank you for choosing CarBooking System! For support, contact us at support@carbooking.com"
...
def send_test_notification():
    ...
    try:
        # Dummy subject and message
        subject = "🔔 Test Notification"
        html_body = f"""
        <html>
        <body>
            <h2>Hello!</h2>
            <p>This is a <strong>test notification</strong> from the Car Booking System admin panel.</p>
        </body>
        </html>
        """
...
def notification_settings():
    ...
    if 'user_id' in session:
        # Get user phone number from DB (modify if your table uses a different name)
        conn = sqlite3.connect('car_booking.db')
        cursor = conn.cursor()
    ...
    if request.method == 'POST':
        # Save the notification preferences to DB here (optional)
